...
plot_metric_across_subscenarios <- function(all_res, all_metrics, metric_y, metric_to_show, plot_boxplots = F){
  ...
  yintercept = case_when(metric_y %in% c("HWE_P_1", "HWE_P_2", "HWE_P_global", 
            ...
            metric_y == "hat_diff_BXY" ~ unique(all_res$diff_BXY)
            )
...
ggsave(filename = paste0(plot_dir_sce, "/BGXk_estimated_across_replicates_x_subsce.pdf"), width = 10, height = 12)
...
ggsave(filename = paste0(plot_dir_sce, "/IV_strength_across_replicates_x_subsce.pdf"), width = 10, height = 3)
...
ggsave(filename = paste0(plot_dir_sce, "/diff_BGXk_across_replicates_x_subsce.pdf"), width = 10, height = 3)
